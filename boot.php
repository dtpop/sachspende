...
rex_yform_manager_dataset::setModelClass('rex_saspe_things', saspe_things::class);
rex_yform_manager_dataset::setModelClass('rex_saspe_order', saspe_order::class);
...
if (rex::isFrontend()) {
    if (rex_request('addon') == 'saspe' && rex_request('action') == 'optin' && rex_request('key','string','')) {
        ...
        if ($item) {
            $item->optin = 1;
            $item->save();
            rex_redirect(rex_config::get('sachspende','confirmation_page'));
        }
    }
}
